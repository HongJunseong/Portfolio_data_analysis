...
- **프로젝트 소개**
  ...
  - 음식과 관련이 있는 키워드로 검색하여 나온 게시글을 크롤링한 뒤, LLM으로 식당명·주소를 정규 추출하고, 해시태그/키워드/CTA 규칙으로 광고 여부를 판별함
  - 결과는 CSV 중심으로 저장하며, 운영 탐색이 필요할 때는 MongoDB 를 옵션으로 사용함
  - XGBoost, LIGHTGBM, RandomForest를 이용하여 규칙 기반 광고 분류 성능을 진단 및 비교함
  - 광고로 판별된 게시글들을 추천 식당 목록에서 제외하여 사용자에게 더욱 신뢰할 수 있는 정보를 제공함
  - Kakao Map REST API로 주소를 표준화하고 위경도·행정동을 매핑, 거리+문자 유사도로 지점 중복을 정리하고, 지도를 생성하여 나타냄
